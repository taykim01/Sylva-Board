...
import { ChatbotMessage, useDemoAIChat } from "@/hooks/use-demo-ai-chat";
...
export function DemoAiChatbot() {
  ...
  const { messages, loading, error, sendMessage, clearMessages, clearError } = useDemoAIChat();
  ...
  const handleSourceClick = (noteId: string) => {
    // Navigate to the note in demo
    window.location.href = `/demo?note_id=${noteId}`;
  };
