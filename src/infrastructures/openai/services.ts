 ...
 * The title and content are combined to provide richer semantic context for better search results
 */
export async function generateNoteEmbedding(
  note: Pick<Tables<"note">, "title" | "content">,
): Promise<EmbeddingResponse> {
  // Combine title and content with clear separation
  // Title is given more weight by being mentioned first
  const combinedText = [note.title?.trim() || "", note.content?.trim() || ""].filter(Boolean).join("\n\n");

  // Don't generate embedding for completely empty notes
  if (!combinedText.trim()) {
    return { embedding: [], error: "Note has no content to embed" };
  }
...
export async function generateChatCompletion(
  ...
  try {
    ...
    const apiMessages: ChatMessage[] = [{ role: "system", content: systemMessage }, ...messages];
